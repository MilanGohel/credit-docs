...
description: API_KEY (server) + Bootstrap Token (client) + internal access/refresh.
---

## The security model (read this once, save yourself pain)

<Callout type="warning" title="Rule #1: API_KEY is server-only">
...
1) **Backend authenticates with API_KEY**  
...
## Bootstrap token mint (SDK integrations)

Your backend mints a short-lived bootstrap token for the SDK:
...
The `allowed_parent_origin` must match your allowlisted origin. Keep it server-defined.

<Callout type="info" title="Non-SDK flows">
See [Tenant Onboarding Guide](./docs/tenant-onboarding) for exchange, provisioning, journey selection, refresh, and logout endpoints.
</Callout>
