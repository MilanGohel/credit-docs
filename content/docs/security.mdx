...
<Callout type="error" title="Never do this">
  - Put API_KEY in `.env` that gets shipped to the browser - Call token mint
  endpoints directly from the browser using API_KEY - Log tokens in your
  server logs
</Callout>
...
When minting bootstrap tokens, set `allowed_parent_origin` to your allowlisted parent origin.
Keep this value server-defined; do not accept it from the browser.
...
## postMessage origin checks (manual iframe embeds)

If you embed the iframe manually (no SDK), enforce strict origin checks on both sides:

- Parent only accepts messages from the iframe origin
- Iframe only accepts messages from the parent origin

See [Tenant Onboarding Guide](./docs/tenant-onboarding) for the v1 postMessage envelope and handshake sequence.
