...
export function ApplyForLoan() {
  ...
  async function onClick() {
    ...
    const res = await fetch("/api/credit/bootstrap-token", { method: "POST" });
  ...
  return (
    <AarthikShell
      ...
      triggerProperties={{
        component: (
          <button className="px-4 py-2 rounded-md border" onClick={onClick}>
            Apply for a loan
          </button>
        ),
      }}
      // Pass bootstrapToken into the SDK so the iframe can load the correct
      // journey context.
...
<Callout type="info" title="SDK handles the auth lifecycle">
Once you pass `bootstrapToken`, the SDK handles exchange, refresh, provisioning, and journey selection.
For manual flows, see [Tenant Onboarding Guide](./docs/tenant-onboarding).
</Callout>
