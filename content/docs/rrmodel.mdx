...
```json
{
  ...
  borrower_provider_id: string; // Required. Tenant-provided borrower identifier.
  contact_number: string; // Required. E.164 format accepted (+91 only).
  pan?: string; // Optional. Permanent Account Number (PAN).
};
```
